...
(function (window, document) {
  ...
  async function loadBusinesses(
    ...
    /* =====================================================
       SELECTION
       -----------------------------------------------------
       forcedSelection is used by direct SEO category
       routes so the URL remains the source of truth.
    ===================================================== */

    if (
      forcedSelection &&
      typeof forcedSelection === "object"
    ) {

      state.selectedState =
        clean(
          forcedSelection.state
        );

      state.selectedDistrict =
        clean(
          forcedSelection.district
        );

      state.selectedCategory =
        clean(
          forcedSelection.category
        );

      if (
        forcedSelection.sort
      ) {
    ...
    console.debug(
      "[UBnux Businesses] loadBusinesses selection:",
      {
        state:
          state.selectedState,
        district:
    ...
    if (
      ...
      console.warn(
        "[UBnux Businesses] Invalid selection. Request skipped.",
        {
          state:
            state.selectedState,
          district:
