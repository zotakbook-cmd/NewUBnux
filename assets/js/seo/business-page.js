...
(function (window, document) {
  ...
  function recoverTemplateVisibility() {
    ...
    /*
     * The business template is mounted dynamically inside the UBnux
     * shell.  Its top-level MAIN must participate in normal document
     * flow; otherwise a template whose sections use absolute/fixed
     * positioning can leave the parent at zero height.
     */
    var templateMain =
    ...
    if (templateMain) {
      ...
      templateMain.style.position = "relative";
    ...
    /*
     * Keep top-level template sections in normal flow.  We intentionally
     * do NOT touch descendants because hero/header internals legitimately
     * use absolute positioning.
     */
    var flowParent = templateMain || content;
    ...
    var mainRect =
      templateMain
        ? templateMain.getBoundingClientRect()
        : null;
    ...
    /*
     * One more layout pass after the browser has applied template CSS.
     * This is important for dynamically injected templates because
     * fonts/images/CSS can change the layout after the first measurement.
     */
    if (typeof window !== "undefined" && typeof window.requestAnimationFrame === "function") {
      window.requestAnimationFrame(function () {
        ...
        var liveContent = getContent();
        var liveMain = liveContent.querySelector("main");

        if (liveRoot) {
          liveRoot.hidden = false;
          liveRoot.removeAttribute("hidden");
          liveRoot.style.display = "block";
          liveRoot.style.visibility = "visible";
        }

        liveContent.style.display = "block";
        liveContent.style.visibility = "visible";
        liveContent.style.opacity = "1";
        liveContent.style.minHeight = "100vh";

        if (liveMain) {
          liveMain.style.display = "block";
          liveMain.style.position = "relative";
          liveMain.style.visibility = "visible";
          liveMain.style.opacity = "1";
          liveMain.style.width = "100%";
          liveMain.style.height = "auto";
          liveMain.style.minHeight = "100vh";
        }

        var finalContentRect = liveContent.getBoundingClientRect();
        var finalMainRect = liveMain ? liveMain.getBoundingClientRect() : null;

        log("Template layout pass complete.", {
          contentHeight: finalContentRect.height,
          mainHeight: finalMainRect ? finalMainRect.height : 0,
          visible: finalContentRect.height > 0
        });
      });
    }
